...
enum error_t numbers_array_memory_increase(struct numbers_array * numbers) {
  assert(numbers);
  enum error_t error = EXIT_SUCCESS;
  size_t new_capacity = INCREASE_FACTOR * numbers->capacity;
  // Si es llamado, se ocupa más espacio
  // container as a new array with 10 times more memory
  struct number_container * new_contained_numbers =  (struct number_container*)
    calloc (new_capacity, sizeof(struct number_container));

  if (new_contained_numbers) {
    // Debe copiarse los datos
    // copy data from old to new container
    for (size_t i = 0; i < numbers->count && (error == ERROR_SUCCESS); ++i) {
      error = number_container_init(&new_contained_numbers[i]);
      error = number_container_set_number(&new_contained_numbers[i],
        numbers->contained_numbers[i].number);
      new_contained_numbers[i].see_sums =
        numbers->contained_numbers[i].see_sums;
    }
    // free old container
    for (uint64_t i = 0; i < numbers->capacity &&
      (error == ERROR_SUCCESS); ++i) {
       error = number_container_destroy(&numbers->contained_numbers[i]);
    }
    // free old container
    free(numbers->contained_numbers);
