...
enum error_t number_container_set_number(
  ...
  if (number % 2 != 0) {  // Se inicializa como 0
    // is_even := 0
...
enum error_t number_container_memory_increase(
  ...
  assert(num_container);
  enum error_t error = ERROR_SUCCESS;
  size_t new_capacity = INCREASE_FACTOR * num_container->capacity;
  // Si es llamado, se ocupa más espacio
  // container as a new array with 10 times more memory
  uint64_t * new_goldbach_sums =
    (uint64_t*) calloc (new_capacity, sizeof(uint64_t));
  if (new_goldbach_sums) {
    for (size_t i = 0; i < num_container->count; ++i) {
      // copy data from old to new container
      new_goldbach_sums[i] = num_container->goldbach_sums[i];
    }
    // free old container
    free(num_container->goldbach_sums);
    num_container->goldbach_sums = new_goldbach_sums;
